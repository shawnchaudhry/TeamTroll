...
public class Transaction extends JPanel {
    ...
    public Transaction(Player player, Market market) {
        ...
        transLabel = new JLabel("Transaction: $" + balance + " + (" + change + ") = $"
                + getNewBalance() + "      Cargo Space Remaining: " + (player.getShip().getCargoBay() - numItems()));
     ...
     * 
     * @param String good The name of the good.
     * @param int price The price of the good.
     */
    public int add(String good, int price) {
        if(market.buy() && 0 < balance + change - price && numItems() <= player.getShip().getCargoBay()) {
            trans.put(good, trans.get(good) + 1);
            change -= price;
            //System.out.println("add called, change: " + change);
        }
        else if(!market.buy() && trans.get(good)*-1 < player.getInventory().get(good)) {
            trans.put(good, trans.get(good) - 1);
            change += price;
        }
    ...
    /**
     * Calculates the new balance of the player's money should the transaction occur.
     *
     * @return int The new balance of the player's money should the transaction occur.
     */
    ...
    private void refresh() {
        transLabel.setText("Transaction: $" + balance + " + (" + change + ") = $" + getNewBalance() 
                + "      Cargo Space Remaining: " + (player.getShip().getCargoBay() - player.numItems() - numItems()));
    }
